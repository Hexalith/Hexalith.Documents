...
/// </summary>
public class GetDataManagementSummariesHandler : RequestHandlerBase<GetDataManagementSummaries>
{
    private readonly IIdCollectionFactory _collectionFactory;
    private readonly IProjectionFactory<DataManagementSummaryViewModel> _projectionFactory;

    /// <summary>
    /// Initializes a new instance of the <see cref="GetDataManagementSummariesHandler"/> class.
    /// </summary>
    /// <param name="collectionFactory">The collection factory.</param>
    /// <param name="projectionFactory">The projection factory.</param>
    public GetDataManagementSummariesHandler(IIdCollectionFactory collectionFactory, IProjectionFactory<DataManagementSummaryViewModel> projectionFactory)
    {
    ...
    public override async Task<GetDataManagementSummaries> ExecuteAsync(GetDataManagementSummaries request, Metadata metadata, CancellationToken cancellationToken)
    {
        ...
        List<Task<DataManagementSummaryViewModel?>> summaryTasks = [];
        ...
        DataManagementSummaryViewModel?[] results = await Task.WhenAll(summaryTasks).ConfigureAwait(false);

        IEnumerable<DataManagementSummaryViewModel> queryResult = results.Where(p => p is not null).OfType<DataManagementSummaryViewModel>();
